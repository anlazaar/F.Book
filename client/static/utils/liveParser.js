...
window.onpopstate = function () {
  ...
  window.location.href = "https://fbook-production.up.railway.app/";
  window.reload();
};
  ...
  form.addEventListener("submit", async (event) => {
    ...
    if (form.method === "post") {
      const result = await Promise.resolve(
        ExecPostRequest(form.action, formValues)
      );
      if (result === false) {
...
posts.forEach((post) =>
  post.addEventListener("click", async () => {
    const result = await Promise.resolve(
      ExecGetRequest("https://fbook-production.up.railway.app/post", {
        id: post.getAttribute("card-id"),
      })
    );
    history.pushState(
      null,
      "",
      "https://fbook-production.up.railway.app/post?id=" +
        post.getAttribute("card-id")
    );
    if (result === false) {
      location.reload();
      return;
